...
def main():
    ...
    parser.add_argument('--subnet_mode', type=str, default="both", help='')
    parser.add_argument('--sparsity_attn', type=float, default=0.5, help='')
    parser.add_argument('--sparsity_mlp', type=float, default=0.5, help='')
    ...
    if args.tuning_mode == 'finetune':
    ...
    elif args.tuning_mode == 'subnettune':
        ...
        config = AutoConfig.from_pretrained(args.model_name_or_path, cache_dir=args.cache_dir)
        print(config)
        model_trained = model_class.from_pretrained(args.model_name_or_path, config=config, cache_dir=args.cache_dir)
        print(model_trained)
        ...
        for name, param in trained_param.items():
            if "attn" in name or "mlp" in name:
                if (args.subnet_mode == "attn" and "mlp" in name) or (args.subnet_mode == "mlp" and "attn" in name):
                    continue
                ...
                if name in ori_param.keys():
                    ...
                    if "attn" in name:
                        j = int((1 - args.sparsity_attn) * score.numel())
                        flat_mask = mask.flatten()
                        flat_mask[idx[:j]] = 0
                        flat_mask[idx[j:]] = 1
                    elif "mlp" in name:
                        j = int((1 - args.sparsity_mlp) * score.numel())
                        flat_mask = mask.flatten()
                        flat_mask[idx[:j]] = 0
                        flat_mask[idx[j:]] = 1
    ...
    if args.task_mode == 'data2text':
        ...
        if args.task_mode == 'webnlg':
            ...
            # test_path = "../../data/dart/dart-v1.1.1-full-dev.json"
    ...
    for prompt_idx, prompt_text in enumerate(prompt_text_lst):
        ...
        elif args.tuning_mode == 'finetune' or 'subnettune':
            print(decode_mode)
            if decode_mode == 'nucleus':
    ...
    if args.task_mode == 'data2text':
        out_file_eval = curr_dir
        print(out_file_eval, '\n', gold_dir, '\n', curr_dir)
        os.system("python /home/mingyuan/Lab/Mingyuan/SubNet/PrefixTuning/e2e-metrics/measure_scores.py "
        ...
        os.system("bash /home/mingyuan/Lab/Mingyuan/SubNet/PrefixTuning/dart/evaluation/run_eval_on_webnlg.sh "
                  "{} {} >> {}".format(curr_dir, tagging, out_file_eval))
    
    elif args.task_mode == 'triples':
        out_file_eval = curr_dir + '_eval'
        print(out_file_eval, '\n', gold_dir, '\n', curr_dir)
        os.system("bash /home/mingyuan/Lab/Mingyuan/SubNet/PrefixTuning/dart/evaluation/run_eval_on_dart.sh "
                  "{} >> {}".format(curr_dir, out_file_eval))
