"""PyTorch OpenAI GPT-2 model for SubNet."""
...
#         super(SupermaskLinear, self).__init__(in_features, out_features, bias)

#         self.sparsity = sparsity

#         # initialize the scores
#         self.scores = nn.Parameter(torch.Tensor(in_features, out_features))
...
class SupermaskConv1D(nn.Module):
    def __init__(self, nf, nx, sparsity=0.5):
        super().__init__()
        self.nf = nf
        self.sparsity = sparsity
        # print(f"Using sparsity = {self.sparsity}")
        ...
        # Scores for subnet
        self.scores = nn.Parameter(torch.empty(self.weight.size()))
        nn.init.kaiming_uniform_(self.scores, a=math.sqrt(5))
        # nn.init.kaiming_normal_(self.weight, mode="fan_in", nonlinearity="relu")
        
        # Disable gradients for original weights
        self.weight.requires_grad = False

    def forward(self, x):
        # print("Using subnet with sparsity {}.".format(self.sparsity))
...
class Attention_sub(nn.Module):
    def __init__(self, nx, n_ctx, config, scale=False, is_cross_attention=False):
        ...
        self.is_cross_attention = is_cross_attention
        if self.is_cross_attention:
            self.c_attn = SupermaskConv1D(2 * n_state, nx, sparsity=config.sparsity_attn)
            self.q_attn = SupermaskConv1D(n_state, nx, sparsity=config.sparsity_attn)
        else:
            self.c_attn = SupermaskConv1D(3 * n_state, nx, sparsity=config.sparsity_attn)
        self.c_proj = SupermaskConv1D(n_state, nx, sparsity=config.sparsity_attn)
        self.attn_dropout = nn.Dropout(config.attn_pdrop)
        self.resid_dropout = nn.Dropout(config.resid_pdrop)
        self.pruned_heads = set()
...
class MLP_sub(nn.Module):
    def __init__(self, n_state, config):
        ...
        # print("nx:", nx)
        self.c_fc = SupermaskConv1D(n_state, nx, sparsity=config.sparsity_mlp)
        self.c_proj = SupermaskConv1D(nx, n_state, sparsity=config.sparsity_mlp)
        self.act = ACT2FN[config.activation_function]
        self.dropout = nn.Dropout(config.resid_pdrop)
...
class Block(nn.Module):
    def __init__(self, n_ctx, config, scale=False):
        ...
        # print(config.subnet_mode)
        if config.subnet_mode == "attn" or config.subnet_mode == "both":
            self.attn = Attention_sub(hidden_size, n_ctx, config, scale)
        else:
            self.attn = Attention(hidden_size, n_ctx, config, scale)
        self.ln_2 = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon)
        if config.add_cross_attention:
            if config.subnet_mode == "attn" or config.subnet_mode == "both":
                self.crossattention = Attention_sub(hidden_size, n_ctx, config, scale, is_cross_attention=True)
            else:
                self.crossattention = Attention(hidden_size, n_ctx, config, scale, is_cross_attention=True)
            self.ln_cross_attn = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon)
        if config.subnet_mode == "mlp" or config.subnet_mode == "both":
            self.mlp = MLP_sub(inner_dim, config)
        else:
...
class GPT2LMHeadModel_SubNet(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.task_mode = config._my_arg_task_mode
        if config._my_arg_tune_mode == 'subnettune':
            self.finetune_mode = True
        else:
            assert False, "incorrect tune mode"
