...
#         self.scores = nn.Parameter(torch.Tensor(out_features, in_features))
...
# class SubNetTunning(GPT2PreTrainedModel):
#     def __init__(self, config, deep_param=False):
#         super().__init__(config)
#         # 假设我们想要在最后一个前馈网络层应用SupermaskLinear
#         self.match_n_layer = config.n_layer
#         self.match_n_head = config.n_head
#         self.match_n_embd = config.n_embd // config.n_head
#         self.n_embd = config.n_embd
#         self.supermask_ffn = SupermaskLinear(config.n_embd, config.n_embd, bias=False)

#         if hasattr(config, '_my_arg_tune_mode'):
#             self.tuning_mode = config._my_arg_tune_mode
#         else:
#             self.tuning_mode = 'subnettune'
...
#         # 应用SupermaskLinear到最后一个前馈网络的输出
#         sequence_output = transformer_outputs[0] # 取得主输出
#         sequence_output = self.supermask_ffn(sequence_output)
